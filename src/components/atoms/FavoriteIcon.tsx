...
const FavoriteIcon = () => {
  ...
  if (favorites.includes(stockName)) {
    return <Box size="2rem" color="yellow.300" as={AiFillStar} onClick={() => {removeFavorite(stockName)}}/>
  } else {
    return <Box size="2rem" color="yellow.300" as={AiOutlineStar} onClick={() => {addFavorite(stockName)}}/>
  }
